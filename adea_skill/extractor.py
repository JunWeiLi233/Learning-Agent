...
class DesignSystemExtractor:
    ...
    def extract(self, design_data: Dict[str, Any]) -> DesignSystem:
        ...
        system.principles = design_data.get("principles", [])

        # Extract design language
        system.design_language = design_data.get("design_language", {})
...
This section captures the underlying design language and philosophy.

"""
        # Add design language philosophy
        dl = system.design_language
        if dl:
            md += f"""### Philosophy

{dl.get('philosophy', 'N/A')}

### Visual Style

{dl.get('visual_style', 'N/A')}
...
"""
            for char in dl.get('key_characteristics', []):
                md += f"- {char}\n"

            md += "\n### Do\n\n"
            for item in dl.get('do', []):
                md += f"- {item}\n"

            md += "\n### Do Not\n\n"
            for item in dl.get('do_not', []):
                md += f"- {item}\n"
        ...
        if system.principles:
            ...
            for principle in system.principles:
                md += f"""### {principle.get('name', 'Unnamed')}
...
"""
                ...
                md += f"\n**Application:** {principle.get('application', 'N/A')}\n\n"

        md += """---

*This design system was auto-generated by ADEA (Autonomous Design Engineering & Adaptation)*
*The goal is to teach AI agents to understand design language, not just copy pixels.*
"""
